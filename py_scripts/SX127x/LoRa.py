...
class LoRa(object):
    ...
    def set_mode(self, mode):
        ...
        print("\033[93mLoRa -> %s\033[0m" % MODE.lookup[mode])
        if mode == self.mode:
            return mode
        if self.verbose:
    ...
    def read_payload(self , nocheck = False):
        ...
        #     'INDEX_MSG_STATUS': 0,
        #     'INDEX_SEQUENCE_ID': 0,
        #     'INDEX_DATA_LOCATION': 0,
        #     'INDEX_DATA_RELAY_STATE': 0,
        #     'INDEX_DATA_ERR_CODE': 0,
        ...
        temp = self.spi.xfer([REG.LORA.FIFO] + [0] * rx_nb_bytes)[1:]
        # payload = dict(zip(payload, temp))
        return temp
    ...
    def clear_irq_flags(self,
        ...
        for i, s in enumerate(['CadDetected', 'FhssChangeChannel', 'CadDone', 
            ...
            if this_bit is not None:
                ...
                print('\033[93mClear flag: ' + s + '\033[0m')
        return self.spi.xfer([REG.LORA.IRQ_FLAGS | 0x80, v])[1]
